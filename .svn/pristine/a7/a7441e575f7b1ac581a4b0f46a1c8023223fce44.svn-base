...
namespace Jayrock
{
  ...
  public static class InternetDate
  {
    private static readonly string[] Formats =
      {
        "dd MMM yyyy HH':'mm",
        "dd MMM yyyy HH':'mm':'ss", "ddd, dd MMM yyyy HH':'mm",
    ...
    public static DateTime Parse(string input)
    {
      ...
      if (input.Length < Formats[0].Length)
        ...
        default:
          {
            if (zone.Length < 4)
              throw new FormatException(
                "Length of local differential component must be at least 4 characters (HHMM).");

            try
            {
              zzz = int.Parse(
                zone, NumberStyles.AllowLeadingSign, CultureInfo.InvariantCulture);
            }
      ...
      input = input.Substring(0, zoneSpaceIndex)
        .TrimEnd();
      var time = DateTime.ParseExact(
        input, Formats, CultureInfo.InvariantCulture, DateTimeStyles.AllowInnerWhite);
