...
namespace Vote.Master
{
  public partial class ImagesHeadshots : SecurePage
  {
    ...
    public static string Url_Image_Politician(string politicianKey, int imageWidth,
      int noPhotoWidth = 0)
    {
      ...
      if (noPhotoWidth == imageWidth)
        url += "&Def=1"; // the short form
      else
      {
        url += "&Def=";
        var defaultColumnName = ImageManager.GetPoliticianImageColumnNameByWidth(noPhotoWidth);
    ...
    public static string Politician_PartyCode(PageCache cache, string politicianKey)
    {
      ...
      return string.IsNullOrEmpty(partyKey)
        ? string.Empty
    ...
    private void ClearLabels()
    {
      ...
      LabelPoliticianKey.Text = string.Empty;
      LabelOfficeLevel.Text = string.Empty;
      LabelPolitician.Text = string.Empty;
      LabelOffice.Text = string.Empty;
      LabelRows.Text = string.Empty;
    }
    ...
    private void GetNextPoliticianForStateOption()
    {
      try
      {
        ...
        if ((politicianKeys == null) || (politicianIndex == int.MinValue))
        {
          ...
          if (stateCode == string.Empty) throw new VoteUIException("A State Code is required.");
    ...
    private void GetPoliticianForSingleOption()
    {
      ...
      if (item == null) throw new VoteUIException("Politician for PoliticianKey not found.");
    ...
    private void SetLabelsFromItem(PoliticianImagesInfo item)
    {
      ...
      LabelOfficeLevel.Text =
        item.OfficeLevel.ToString(CultureInfo.InvariantCulture);
    ...
    protected override void OnPreRender(EventArgs e)
    {
      try
      {
        ...
        var isPoliticianKeyValid =
          !string.IsNullOrWhiteSpace(LabelPoliticianKey.Text);
    ...
    protected void ButtonNext_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        Msg.Text =
          Ok("Upload any desired headshot or profile images." +
    ...
    protected void ButtonUploadPicture_ServerClick(object sender, EventArgs e)
    {
      try
      {
        ...
        if ((postedFile == null) || (postedFile.ContentLength == 0) ||
          string.IsNullOrEmpty(politicianKey)) return;
        Size originalSize;
      ...
      EventArgs e)
    {
      try
      {
        ...
        TextBoxOfficeLevel.Text = string.Empty;
        TextBoxPoliticianKey.Text = string.Empty;
        ClearLabels();
        ClearStateOptionStatus();
        EnableControls();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      try
      {
        Page.Title = "Headshots Maintenance";
      ...
      // Helper class that manages the info for each politician
    {
      ...
      private void ApplyAdditionalCoding(PageCache pageCache)
      {
        ...
        if (Offices.IsValid(_OfficeKey)) OfficeLevel = pageCache.Offices.GetOfficeLevel(_OfficeKey);
        else
        {
          if (string.IsNullOrEmpty(_OfficeKey)) _OfficeKey = "????";
      ...
      public static List<PoliticianImagesInfo> GetNextOutOfDateHeadshot(
        int officeLevel, DateTime profileOriginalDate, string politicianKey)
      {
        ...
        list =
          list.Where(
              item => item.IsGreater(officeLevel, profileOriginalDate, politicianKey))
      ...
      public static PoliticianImagesInfo GetHeadshotDataForPolitician(
      ...
      private bool IsGreater(int officeLevel, DateTime profileOriginalDate,
        string politicianKey)
      {
        return (OfficeLevel > officeLevel) ||
