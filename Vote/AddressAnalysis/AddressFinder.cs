...
namespace Vote
{
  ...
  public class AddressFinderResult
  {
    ...
    {
      email = email.SafeString().Trim();
      try
      {
        var zip5 = (Zip5S != null) && (Zip5S.Length > 0) ? Zip5S[0] : null;
        if (Success)
          LogAddressesGoodNew.Insert(DateTime.UtcNow, email, OriginalInput, null,
            ParsedAddress, ParsedCity, State, zip5, Zip4, false, false);
        else
          LogAddressesBadNew.Insert(DateTime.UtcNow, email, OriginalInput, ErrorMessage);
      }
      // ReSharper disable once EmptyGeneralCatchClause
      catch {} // don't allow a failure to kill the request
    }
  ...
  public class AddressFinder
  {
    ...
    //  public List<StreetAddressParseInfo> ParseInfoList;
    ...
    //    sb.Append("Zip5: ");
    //    sb.Append(Zip5.ToStringOrNull());
    //    sb.Append(Environment.NewLine);

    //    sb.Append("ZipPlus4: ");
    //    sb.Append(ZipPlus4.ToStringOrNull());
    //    sb.Append(Environment.NewLine);

    //    sb.Append("Additional Zip5s: ");
    //    if (Zip5s != null)
    //      sb.Append(string.Join(", ", Zip5s));
    //    sb.Append(Environment.NewLine);

    //    sb.Append("State: ");
    //    sb.Append(StateCode.ToStringOrNull());
    //    sb.Append(Environment.NewLine);

    //    if (ParseInfoList != null)
    //      foreach (var info in ParseInfoList)
    //      {
      ...
      string valueToTry, bool useMetaphone, out bool abbreviationMatch)
    {
      ...
      // Disabled because currently there are less than 10 abbreviations that are not
      // also included as a main AliasName.
      //// If no results, try again using CityAliasAbbreviation. We want an exact match here.
      //if (table == null || table.Count == 0)
      //{
      //  valueToTry = valueToTry.Replace('%', ' '); // no wildcards here
      //  if (stateCode == null) // no state available
      //  {
      //    if (useMetaphone) // don't do this for metaphone
      //      table = null;
      //    else
      //      table =
      //        ZipCitiesDownloaded.GetCityAliasesDataByCityAliasAbbreviation(valueToTry);
      //  }
      //  else // use the state we have
      //  {
      //    if (useMetaphone)
      //    {
      //      string metaphone = DoubleMetaphone.EncodePhrase(valueToTry);
      //      table =
      //        ZipCitiesDownloaded.GetCityAliasesDataByStateMetaphoneAliasName(stateCode,
      //         metaphone);
      //    }
      //    else
      //      table =
      //        ZipCitiesDownloaded.GetCityAliasesDataByStateCityAliasAbbreviation(stateCode,
      //         valueToTry);
      //  }
      //  if (table != null && table.Count != 0)
    ...
    //private List<StreetAddressParseInfo> GetParsePossibilities()
    //{
    //  // Like TryParsePossibilities, but returns all parse possibilities.
    //  // For testing, debugging
    //  var list = new List<StreetAddressParseInfo>();

    //  if (GetZip5s().Length > 0)
    //    foreach (var parts in OriginalParts)
    //      foreach (var info in PossibleStreetAddressPatterns(parts))
    //        list.Add(info.Clone());
    ...
    //private static Dictionary<ZipPlus4, object> CreateZipPlus4List(List<StreetAddressDatabaseInfo> dbList)
    //{
    //  Dictionary<ZipPlus4, object> plus4List = new Dictionary<ZipPlus4, object>();
    //  foreach (var streetInfo in dbList)
    //  {
    //    if (streetInfo.Plus4Low == streetInfo.Plus4High)
    //      plus4List[new ZipPlus4(streetInfo.ZipCode, streetInfo.Plus4Low)] = null;
    //    else
    //    {
    //      int low = int.Parse(streetInfo.Plus4Low);
    //      int high = int.Parse(streetInfo.Plus4High);
    //      // For reasonability, we cap the range at 200
    //      high = Math.Min(high, low + 199);
    //      for (int n = low; n <= high; n++)
    //        plus4List[new ZipPlus4(streetInfo.ZipCode, n.ToString().ZeroPad(4))] = null;
    //    }
    //  }
    //  return plus4List;
    //}

    //private static Dictionary<LdsInfo, object> GetUniqueLdsInfo(Dictionary<ZipPlus4, object> plus4List)
    //{
    //  var ldsInfoDictionary = new Dictionary<LdsInfo, object>();
    //  if (plus4List.Count > 0)
    //  {
    //    var keys = plus4List.Keys.OfType<ZipPlus4>();
    //    var table = Uszd.GetDataByZipPlus4List(keys);
    //    foreach (var row in table)
    //    {
    //      string stateCode = StateCache.StateCodeFromLdsStateCode(row.LdsStateCode);
    //      LdsInfo ldsInfo = new LdsInfo(stateCode, row.Congress, row.StateSenate,
    //        row.StateHouse, row.County);
    //      if (ldsInfo != null)
    //        ldsInfoDictionary[ldsInfo] = null;
    //    }
    //  }
    //  return ldsInfoDictionary;
    //}
    ...
    //private ParseResults ParsePrivate(string input, string defaultState)
    //{
    //  // The parse-only version of FindPrivate
    //  ParseResults results = new ParseResults();

    //  InitParse(input, defaultState);

    //  results.Zip5 = Zip5;
    //  if (Zip4 != null)
    //    results.ZipPlus4 = Zip5 + '-' + Zip4;
    //  if (CityZip5CodesFromParse != null)
    //    results.Zip5s = CityZip5CodesFromParse.ZipCodes;
    //  string stateCode = BestState;
    //  if (BestState == null)
    //    stateCode = CityZip5CodesFromParse.StateCode;
    //  results.StateCode = stateCode;

    //  if (!ResultIsComplete)
    //    results.ParseInfoList = GetParsePossibilities();
    ...
    public static AddressFinderResult Find(string input, string defaultState, bool remember = true) => 
      new AddressFinder().FindPrivate(input, defaultState, remember);
