...
//      }
    ...
    function editRecipientSelections() {
      ...
      // no further editing necessary for PartyOfficials, Volunteers, WebsiteVisitors or Donors
      if (recipientType === "PartyOfficials" || recipientType === "Volunteers" ||
    ...
    function restoreSelectRecipientOptions(o, restoreInfo) {
      ...
      $.each(["SampleBallots", "SharedChoices", "Donated",
    ...
    function addPreviewOptionsToData(data) {
      ...
      data.politicianKey = $('.candidate-dropdown', $previewOptions).val() || $('.politician-key-hidden', $previewOptions).val();
    ...
    function populatePreviewElectionDropdown(val, altVal) {
      ...
      var localCode = $(".local-dropdown", $previewSampleTab).val();
      //
      // This is experimental -- don't use county and local codes 
      ...
      //val = val.substr(0, 12);
      //if (altVal) altVal = altVal.substr(0, 12);
      ...
      util.ajax({
        ...
        success: function (result) {
          ...
          {
            selected += countyCode + localCode;
            altSelected += countyCode + localCode;
          }
          ...
          if (altSelected && $electionDropdown.val() != val &&
    ...
    function onClickEmailAddButton(event) {
      ...
      if ($scroller.hasClass("bulk-email-options-from") && !(/@vote-usa\.org$/i.test(email))) {
        $email.select();
