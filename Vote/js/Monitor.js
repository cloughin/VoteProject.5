define(["jquery", "vote/util", "jqueryui", "textchange", "tiptip", "ajaxfileupload"],
 function ($, util) {
   ...
   var disableUi = function (ajaxMessage) {
     ...
     //    });
     //    if (global.buttonall) $$(global.buttonall).addClass("disabled");
   };
   ...
   var onBeginAjax = function (group, ajaxMessage) {
     ...
     //setAjaxSpinnerEnabling(true, group);
   ...
   var onEndAjax = function (group) {
     //setAjaxSpinnerEnabling(false, group);
     enableUi();
   ...
   //  var setAjaxSpinnerEnabling = function (state, group) {
   //    var ajaxloader = group && group.ajaxloader ? group.ajaxloader : global.ajaxloader;
   //    if (ajaxloader)
   //      if (state)
   ...
   TextData.prototype.handles = function ($data) {
     ...
     return type === "text" || type === "password" || type === "hidden";
   };
