  ...
  public partial class SecurePage
  {
      ...
      string asteriskId, string asteriskClassName, string asteriskTitle, bool disabled = false)
    {
      var textarea = CreateTextAreaInputElement(textboxId, textboxClassName, value, placeholder,
        ...
        asteriskClassName, asteriskTitle, disabled);
      parent.Controls.Add(textarea);
      return textarea;
    }
    ...
    //public static HtmlContainerControl CreateTimeSpanInputElement(
    //  string controlId, string controlClassName, TimeSpan? value, string[] toolTips,
    //  bool required, string fieldName, string outerClassName, string asteriskId,
    //  string asteriskClassName, string asteriskTitle, bool includeHours = false)
    //{
    //  var element = CreateContainer(null, outerClassName);
    //  element.AddCssClasses("input-element");
    ...
    //  {
    //    var totalMinutes = value.Value.Minutes;
    //    var totalHours = value.Value.Hours + value.Value.Days * 24;
    //    if (includeHours)
    //      hours = totalHours.ToString(CultureInfo.InvariantCulture);
    //    else
    //    {
    //      totalMinutes += totalHours * 60;
    //    }
    //    minutes = totalMinutes.ToString(CultureInfo.InvariantCulture);
    //    seconds = value.Value.Seconds.ToString(CultureInfo.InvariantCulture);

    //  }

    //  var databox = AddContainer(element, controlId, controlClassName + " databox timespan");
    //  if (includeHours)
    //  {
    //    var hoursbox = AddTextBox(
    //      databox, null, "hours", hours, null, false);
    //    if (toolTips != null && toolTips.Length >= 3 && !String.IsNullOrWhiteSpace(toolTips[2]))
    //    {
    //      hoursbox.ToolTip = toolTips[2];
    //      hoursbox.AddCssClasses("tiptip");
    //    }
    //    new LiteralControl(" : ").AddTo(databox);
    //  }
    //  var minutesbox = AddTextBox(
    //    databox, null, "minutes", minutes, null, false);
    //  if (toolTips != null && toolTips.Length >= 2 && !String.IsNullOrWhiteSpace(toolTips[1]))
    //  {
    //    minutesbox.ToolTip = toolTips[1];
    //    minutesbox.AddCssClasses("tiptip");
    //  }
    //  new LiteralControl(" : ").AddTo(databox);
    //  var secondsbox = AddTextBox(
    //    databox, null, "seconds", seconds, null, false);
    //  if (toolTips != null && toolTips.Length >= 1 && !String.IsNullOrWhiteSpace(toolTips[0]))
    //  {
    //    secondsbox.ToolTip = toolTips[0];
    //    secondsbox.AddCssClasses("tiptip");
    //  }
    //  if (asteriskId != null || asteriskClassName != null || asteriskTitle != null)
    //  {
    //    var asterisk = AddContainer(databox, asteriskId, asteriskClassName);
