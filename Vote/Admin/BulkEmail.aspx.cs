      ...
      private static void BuildSubstitutionTable(BulkEmailPage page)
      {
        ...
        foreach (var group in allSubstitutionGroups)
        {
          ...
          for (var n = 0; n < g.Count; n++)
          {
            ...
            var name = n < g.Count
              ? $"<span class=\"escape\">{key.Substring(0, 2)}</span><span class=\"id\">{key.Substring(2, key.Length - 4)}</span><span class=\"escape\">{key.Substring(key.Length - 2, 2)}</span>"
              : "&nbsp;";
    ...
    private class EmailOptionsTabItem : DataItemBase
    {
        ...
        if (!page.IsPostBack)
        {
          ...
          if (emailData.Count == 1)
          {
            ...
            emails.AddRange(emailData[0].UserEmails.SafeString()
              .Split(',')
              .Select(a => a.Trim())
              .Where(a => a.EndsWith("@vote-usa.org", StringComparison.OrdinalIgnoreCase)));
        ...
        string name, bool isRadio, bool isOption = false)
      {
        ...
        foreach (var email in emailList)
        {
          ...
          new HtmlLabel {InnerText = email}.AddTo(div)
            .Attributes["for"] = id;
        }
      }
    }
