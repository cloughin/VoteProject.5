...
namespace Vote.Controls
{
  public partial class LocalsMenuControl : UserControl
  {
    ...
    private void AddLocal(Control block, string localKey, string localName)
    {
      var a = new HtmlAnchor();
      block.Controls.Add(a);
      a.HRef = Format(
        _HRefTemplateForFormat, StateCode, CountyCode, localKey);
      a.Title = localName;
      ...
      if (!IsPostBack)
      {
        var table =
          LocalDistricts.GetLocalsForCounty(StateCode, CountyCode)
            .Rows.OfType<DataRow>()
            .OrderBy(r => r.LocalDistrict(), new AlphanumericComparer())
            .ToList();
        ...
        else
        {
          ...
          for (var inx = 0; inx < table.Count; inx++)
          {
            ...
            if (inx % countiesPerBlock == 0)
              blockIndex++;
            AddLocal(blockControls[blockIndex], row.LocalKey(), row.LocalDistrict());
          }
        }
      }
    }
