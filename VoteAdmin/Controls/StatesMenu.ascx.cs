...
namespace Vote.Controls
{
  public partial class StatesMenuControl : UserControl
  {
    ...
    private void AddState(Control block, string stateCode, string text = null)
    {
      var a = new HtmlAnchor();
      block.Controls.Add(a);
      a.HRef = Format(_HRefTemplateForFormat, stateCode);
      a.InnerText = text ?? StateCache.GetStateName(stateCode);
    }
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      if (!IsPostBack)
      {
        ...
        if (IncludeAll) AddState(statesBlock, "US", "All States");
