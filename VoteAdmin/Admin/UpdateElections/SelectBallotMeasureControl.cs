...
namespace Vote.Admin
{
  public partial class UpdateElectionsPage
  {
    ...
    private class SelectBallotMeasureControlItem : ElectionsDataItem
    {
      ...
      private static SelectBallotMeasureControlItem[] GetTabInfo(UpdateElectionsPage page)
      {
        ...
        foreach (var item in selectBallotMeasureTabInfo) item.InitializeItem(page);
    ...
    {
      if (table.Count == 0)
        new HtmlLi {InnerHtml = "No ballot measures have been added for this election"}
          .AddTo(ControlSelectBallotMeasureList, "no-ballot-measures");
      else
        foreach (var row in table)
        {
          var referendumLi = new HtmlLi {ID = "referendumorder-" + row.ReferendumKey}.AddTo(
            ControlSelectBallotMeasureList,
            "ballot-measure-desc" + (row.IsReferendumTagForDeletion
              ? " tagged"
              : Empty));
          var icons = new HtmlDiv().AddTo(referendumLi, "icons");
          new HtmlDiv().AddTo(icons, "icon-move tiptip").Attributes["title"] =
            "Drag to reorder the ballot measures";
          var indicator = "&#x25ba;";
          var status = Empty;
          ...
          new HtmlDiv {InnerHtml = indicator}.AddTo(icons, "tiptip icon-status" + status)
    ...
    protected void ButtonSelectBallotMeasure_OnClick(object sender, EventArgs e)
    {
      switch (SelectBallotMeasureReloading.Value)
      {
        case "reloading":
        {
          SelectBallotMeasureReloading.Value = Empty;
          SelectBallotMeasureScrollPosition.Value = Empty;
          SelectedBallotMeasureKey.Value = Empty;
          SetElectionHeading(HeadingAddBallotMeasures);
          HeadingAddBallotMeasuresBallotMeasure.InnerHtml = "No ballot measure selected";
        ...
        default:
          throw new VoteException(
            $"Unknown reloading option: '{SelectBallotMeasureReloading.Value}'");
      }
