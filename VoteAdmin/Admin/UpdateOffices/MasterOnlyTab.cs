...
namespace Vote.Admin
{
  ...
  {
    //protected const bool HideLockClasses = true;
    ...
    private class MasterOnlyTabItem : OfficesDataItem
    {
        ...
        // ReSharper restore UnusedMember.Local
      {
        ...
        //if (HideLockClasses)
          page._MasterOnlySubTabs.Remove(MasterOnlySubTab.LockClasses);
      }
    }
    ...
    private void ChangeOfficeKey()
    {
      try
      {
        ...
        if (!IsNullOrWhiteSpace(newKeyOffice))
        {
          // get rid of all non-alphanumerics
          newKeyOffice = Regex.Replace(newKeyOffice, @"[^\dA-Z]", Empty,
            RegexOptions.IgnoreCase);
          // get rid of leading numerics
          newKeyOffice = Regex.Replace(newKeyOffice, @"^\d+", Empty);
        ...
        if (success && !caseChangeOnly)
        {
          ...
          {
            newKeyItem.Feedback.PostValidationError(newKeyItem.DataControl, newKeyItem.Description +
              " already exists in the following tables: " + Join(", ", existsInTables));
            success = false;
          }
        }
