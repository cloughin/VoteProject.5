  ...
  public partial class UpdateJurisdictionsPage
  {
    ...
    private class ElectionAuthorityTabItem : AllJurisdictionsDataItem
    {
      ...
      private static ElectionAuthorityTabItem[] GetTabInfo(UpdateJurisdictionsPage page)
      {
        var electionAuthorityTabInfo = new[]
        {
          ...
          new ElectionAuthorityTabItem(page) {Column = "Notes", Description = "Notes"}
        };
    ...
    private class ElectionAuthorityContactEmailTabItem : ElectionAuthorityTabItem
    {
      ...
      protected override void AfterUpdate(string newValue)
      {
        ...
        {
          if (Label.Controls[0] is LiteralControl literal)
            literal.Text = literal.Text.Replace(" Email", Empty);
          else
          {
            if (Label.Controls[0] is HyperLink mailLink)
              mailLink.Text = mailLink.Text.Replace(" Email", Empty);
          }
        }
      }
    }
    ...
    protected void ButtonElectionAuthority_OnClick(object sender, EventArgs e)
    {
      switch (ElectionAuthorityReloading.Value)
      {
        case "reloading":
        {
          ElectionAuthorityReloading.Value = Empty;
        ...
        default:
          throw new VoteException(
            $"Unknown reloading option: '{ElectionAuthorityReloading.Value}'");
      }
    }
