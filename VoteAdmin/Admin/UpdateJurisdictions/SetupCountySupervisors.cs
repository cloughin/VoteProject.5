...
namespace Vote.Admin
{
  public partial class UpdateJurisdictionsPage
  {
    ...
    private class CountySupervisorItem
    {
...
#pragma warning restore 649
    ...
    private void LoadCountySupervisorDistricts(
      ICollection<CountySupervisorItem> supervisors)
    {
      CountySupervisorsTableBody.Controls.Clear();
      foreach (var s in supervisors)
      {
        ...
        new TableCell()
          .AddTo(tr, $"delete {s.ErrorClass(nameof(CountySupervisorItem.Delete))}").Controls
          .Add(new HtmlInputCheckBox {Disabled = s.Exists, Checked = s.Delete});
        new TableCell()
          .AddTo(tr, $"exists  {s.ErrorClass(nameof(CountySupervisorItem.Exists))}")
          .Controls.Add(new HtmlInputCheckBox {Disabled = true, Checked = s.Exists});
        new TableCell()
          .AddTo(tr, $"create  {s.ErrorClass(nameof(CountySupervisorItem.Create))}")
          .Controls.Add(new HtmlInputCheckBox {Disabled = s.Exists, Checked = s.Create});
        new TableCell().AddTo(tr, "code").Controls.Add(
          new HtmlInputText {Value = s.Id, Disabled = s.Exists}.AddCssClasses(
            s.ErrorClass(nameof(CountySupervisorItem.Id))));
        new TableCell()
          .AddTo(tr,
            $"in-shapefile  {s.ErrorClass(nameof(CountySupervisorItem.InShapefile))}")
        ...
        new TableCell().AddTo(tr, "name").Controls.Add(
          new HtmlInputText {Value = s.Name}.AddCssClasses(
            s.ErrorClass(nameof(CountySupervisorItem.Name))));
      }
    ...
    private class SetupCountySupervisorsTabItem : JurisdictionsDataItem
    {
      ...
      private CountySupervisorItem[] _Submitted;

      public void PrepareForUpdate()
      {
        _Submitted =
          new JavaScriptSerializer().Deserialize<CountySupervisorItem[]>(
            Page.ControlSetupCountySupervisorsSupervisorsListValue.Value);
      }
      ...
      private static bool ValidateCountySupervisors(DataItemBase item)
      {
        ...
        if (missingIds.Count > 0)
        {
          ...
          foreach (var e in missingIds) e.SetError(nameof(CountySupervisorItem.Id));
        }
        ...
        if (duplicateIds.Count > 0)
        {
          ...
          foreach (var e in g) e.SetError(nameof(CountySupervisorItem.Id));
        }
        ...
        if (stateCode != "DC")
        {
          ...
          if (missingCounties.Count > 0)
          {
            ...
            foreach (var e in missingCounties) e.SetError(nameof(CountySupervisorItem.Id));
          }
        }
        ...
        if (missingNames.Count > 0)
        {
          ...
          foreach (var e in missingNames) e.SetError(nameof(CountySupervisorItem.Name));
        }
    ...
    protected void ButtonSetupCountySupervisors_OnClick(object sender, EventArgs e)
    {
      switch (SetupCountySupervisorsReloading.Value)
      {
        case "reloading":
        {
          SetupCountySupervisorsReloading.Value = Empty;
        ...
        default:
          throw new VoteException(
            $"Unknown reloading option: '{SetupCountySupervisorsReloading.Value}'");
      }
    }
