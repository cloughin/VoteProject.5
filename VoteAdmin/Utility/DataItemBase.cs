...
namespace Vote
{
  ...
  internal abstract class DataItemBase
  {
    ...
    {
      if (templateControl is SecurePage page) return page.Master.FindMainContentControl(id);
    ...
    {
      if (DataControl is HtmlControl htmlControl)
    ...
    protected virtual void SetControlValue(string newValue)
    {
      if (DataControl is HtmlControl htmlControl)
        ...
        {
          var valueId = htmlControl.ID + "Value";
          if (htmlControl.Parent.Controls.Cast<Control>()
            .FirstOrDefault(c => c.ID == valueId) is HtmlInputHidden valueControl)
    ...
    {
      if (o is DateTime dateTime) return ToDisplayDate(dateTime);
    ...
    {
      var validated =
        item.Feedback.ValidateDate(item.DataControl as TextBox, out var success,
    ...
    protected static bool ValidateYouTubeVideoAddressRequired(DataItemBase item)
    {
      if (!ValidateRequired(item)) return false;
      return ValidateYouTubeVideoAddressOptional(item);
    }
  }
  ...
  internal static class DataItemBaseExtensions
  {
    ...
    internal static DataItemBase FindItem(this IEnumerable<DataItemBase> items, string column)
    {
      return items.FirstOrDefault(i => i.Column == column);
    }
