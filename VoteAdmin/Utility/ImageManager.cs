...
namespace Vote
{
  public static class ImageManager
  {
    ...
    private static bool IsColumnNameProfileImage(string columnName)
    {
      ...
      PoliticiansImagesBlobs.Column column;
      if (PoliticiansImagesBlobs.TryGetColumn(columnName, out column))
      {
        var politicianImageInfo =
    ...
    //static readonly Regex MimeRegex
    // = new Regex(@"^(?<type>[-+a-z0-9]+)/(?<subtype>[-+a-z0-9]+)",
    //    RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);

    //private static string GetValidatedContentType(Image image)
    //{
    //  var ok = false;
    //  // Get and validate the contentType;
    //  var contentType = GetContentType(image);
    //  if (contentType != null)
    //  {
    //    var match = MimeRegex.Match(contentType);
    //    if (match.Success)
    //    {
    //      if (match.Groups["type"].Value.Equals("image",
    //        StringComparison.OrdinalIgnoreCase))
    //        ok = true;
    //    }
    //  }
    //  if (!ok)
    //    throw new VoteException("The file type is invalid."
    //     + " Valid file types are jpeg, png, gif, bmp, and tiff.");
    //  return contentType;
    //}
      ...
      out Size originalSize)
    {
      ...
      return UpdatePoliticianImages(politicianKey, imageStream, uploadTime,
        ...
        includeHeadshotResizedImages, false, out originalSize, out notUsed);
    }
