...
namespace Vote
{
  public class UpdateAnswer2
  {
    ...
    public UpdateAnswer2(Usage usage)
    {
      _Usage = usage;
    }
      ...
      string moreClasses, MonitorInstance monitor)
    {
      ...
      new HtmlP { InnerText = "When you Update, the Video Description and Running Time will be automatically retrieved from Facebook." }.AddTo(parent, "url-message");
      ...
      if (_DataTable == null)
      {
        ...
        if (_Usage == Usage.ForIssues)
        {
          ...
          table = AnswersView.GetAllDataByPoliticianKeyNew(page.PoliticianKey, officeKey).Rows.OfType<DataRow>().Where(
            r => r.IssueLevel() != Issues.IssueLevelAll);
        }
