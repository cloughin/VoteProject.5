...
namespace DocumentServices.Modules.Readers.MsgReader.Header
{
	...
	internal static class HeaderFieldParser
    {
        ...
        var value = Decode.Utility.RemoveQuotesIfAny(keyValuePair.Value.Trim());
				switch (key)
				{
					case "":
		...
		public static ContentDisposition ParseContentDisposition(string headerValue)
		{
            ...
            contentDisposition.FileName = EncodedWord.Decode(Decode.Utility.RemoveQuotesIfAny(value));
						break;
            ...
            var creationDate = new DateTime(Rfc2822DateTime.StringToDate(Decode.Utility.RemoveQuotesIfAny(value)).Ticks);
						contentDisposition.CreationDate = creationDate;
						break;

					case "MODIFICATION-DATE":
            var midificationDate = new DateTime(Rfc2822DateTime.StringToDate(Decode.Utility.RemoveQuotesIfAny(value)).Ticks);
						contentDisposition.ModificationDate = midificationDate;
						break;

					case "READ-DATE":
            var readDate = new DateTime(Rfc2822DateTime.StringToDate(Decode.Utility.RemoveQuotesIfAny(value)).Ticks);
						contentDisposition.ReadDate = readDate;
						break;

					case "SIZE":
            contentDisposition.Size = int.Parse(Decode.Utility.RemoveQuotesIfAny(value), CultureInfo.InvariantCulture);
						break;

					default:
						if(key.StartsWith("X-"))
						{
              contentDisposition.Parameters.Add(key, Decode.Utility.RemoveQuotesIfAny(value));
							break;
						}
