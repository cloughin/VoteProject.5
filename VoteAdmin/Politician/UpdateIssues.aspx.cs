...
namespace Vote.Politician
{
  public partial class UpdateIssuesPage : SecurePoliticianPage
  {
    ...
    private readonly UpdateAnswer _UpdateAnswer =
      new UpdateAnswer(UpdateAnswer.Usage.ForIssues);
    ...
    private readonly MonitorFactory _MonitorFactory = new MonitorFactory(MonitorPrefix);
    ...
    private void BuildTabTable()
    {
      ...
      var issueGroups = table.GroupBy(g => g.First().IssueGroupKey())
        .OrderBy(group => group.First().First().IssueGroupOrder()).ToList();
      ProcessCategories(issueGroups);
    }

    private HtmlContainerControl CreateCategory(DataRow row, Control horizontalTabs)
    {
      // create panel
    ...
    private HtmlContainerControl CreateIssue(DataRow row, Control tabs,
      Control container)
    {
      if (_IssueTabFont == null)
        // We use bold as worst-case
        _IssueTabFont = new Font(IssueTabFontFamily, IssueTabFontSize, FontStyle.Bold);
      ...
      var verticalTab = AddVertTab(tabs, "#" + panel.ClientID,
        BreakForTab2(row.Issue(), _IssueTabFont, IssueTabMaxWidth), 
        tabKey + " vcentered-tab");
    ...
    private static PlaceHolder GetMainTabLabel(DataRow row)
    {
      ...
      var formattedHeading = ValidateBreaks(row.IssueGroupHeading(), LinesInCategoryHeading,
        true, "Too many lines in category heading: " + row.IssueGroupKey());
      new LiteralControl(formattedHeading) {EnableViewState = false}.AddTo(container);
      ...
      new HtmlSpan {EnableViewState = false, InnerHtml = formattedSubHeading}
        .AddTo(container);
    ...
    private void ProcessCategories(
      IEnumerable<IGrouping<string, IGrouping<string, DataRow>>> categories)
    {
      var horizontalTabs =
        AddTabContainer(MainTabsPlaceholder, null, "tabs htabs unselectable");
      ...
      foreach (var category in categories)
      {
        ...
        var issues = category.GroupBy(g => g.First().IssueKey())
          .OrderBy(group => group.First().First().IssueOrder()).ToList();
    ...
    private void ProcessIssues(
      IEnumerable<IGrouping<string, IGrouping<string, DataRow>>> issues,
      Control container)
    {
      ...
      foreach (var issue in issues)
      {
        ...
        var questions = issue.OrderBy(g => g.First().QuestionOrder()).ToList();
        ProcessQuestions(questions, accordion);
        if (verticalTabs.Controls.Count >= MaxIssuesPerCategory) return;
      }
    }

    private void ProcessQuestions(IEnumerable<IGrouping<string, DataRow>> questions,
      Control container)
    {
    ...
    protected override void OnPreRender(EventArgs e)
    {
      ...
      if (PageCachingSubHeadingWithHelp.FindTemplateControl("ViewIssuesLink") is HtmlAnchor
        viewIssuesLink) viewIssuesLink.HRef = PoliticianIssuePageUrl;
