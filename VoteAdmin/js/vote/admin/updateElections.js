define(["jquery", "vote/adminMaster", "vote/util", "monitor",
  ...
  function ($, master, util, monitor, navigateJurisdiction, electionControl, officeControl,
   managePoliticiansPanel) {
    ...
    function afterUpdateGroup(group) {
      if (!group) return;
      switch (group.group) {
    ...
    function onToggleElectionControl() {
    ...
    function getPartyPrimariesWithOfficesFailed() {
    ...
    function initRequestAddElection(group) {
      ...
      if (isStatePrimary) {
        ...
        if (thisParty == "X") {
          ...
          warning = "There is already a non-partisan primary on the same day. Are you sure you want to add a party primary?";
        }
    ...
    var afterUpdateContainerAddCandidates = function() {
      if (!officeControl.isInitialized()) {
        // wait a bit
        setTimeout(function() { afterUpdateContainerAddCandidates(); }, 500);
        return;
      }
      var officeKey = officeControl.getSelectedOffice();
      if (officeKey) {
        // if the office key is virtual, substitute the CountyCode or LocalKey
         if (officeKey.substr(2, 5) === "#####") {
           officeKey =
             officeKey.substr(0, 2) + $.queryString("local") + officeKey.substr(7);
         } else if (officeKey.substr(2, 3) === "###") {
           officeKey =
             officeKey.substr(0, 2) + $.queryString("county") + officeKey.substr(5);
         }
        $("#tab-addcandidates .edit-office-button")
          .attr("href", "/admin/updateoffices.aspx?office=" + officeKey);
        $("#tab-addcandidates .head-col-2-content").removeClass("hidden");
        var positions = officeControl.getOfficePositions();
        var msg = "";
        if (positions)
          msg = positions + " position" + (positions == 1 ? "" : "s");
        $("#tab-addcandidates .positions").text(msg);
        $(".setup-ad-button")
          .attr("href", "/admin/SetupComparePageBannerAd.aspx?election=" +
            getSelectedElection() + "&office=" + officeKey);
      }
    };
        ...
        if ($(".runoff-checkbox", $dropdowns).prop("checked")) {
          ...
          $dropdowns.find(".winners-dropdowns select").each(function () {
            if ($(this).text().indexOf('◄') < 0) {
              // Intend to use current value always
    ...
    function onSelectedWinnerChanged() {
      this.blur();
      var $this = $(this);
    ...
    function onClickGetReport() {
      ...
      if ($("#get-report-in-new-window-checkbox").prop("checked")) {
        window.open("/admin/election.aspx?&election=" + electionKey, "view");
        return;
      }

      $("#ElectionIFrame").attr("src", "/admin/election.aspx?iframe=y&election=" + electionKey).removeClass("hidden");
    }
    ...
    function initRequestMasterOnly(group) {
      ...
      if (group.children) {
        // warnings
        // find the various groups
        var electionToCopy = null;
        var enableCopyPrimaryWinnersGroup = null;
        var deleteElectionGroup = null;
        $.each(group.children, function () {
          switch (this.group) {
            case "mc-group-masteronly-electiontocopy":
              electionToCopy = this;
            break;
            case "mc-group-masteronly-enablecopyprimarywinners":
            enableCopyPrimaryWinnersGroup = this;
            break;
          case "mc-group-masteronly-deleteelection":
            deleteElectionGroup = this;
            break;
          }
        });

        if (electionToCopy && electionToCopy._current) {
          util.confirm("You are about to add ALL candidates (not just winners) from\n" +
            electionToCopy._current + " to\n" + electionControl.getElectionDesc() +
            ".\n\nOk?",
            function (button) {
              if (button === "Ok") {
                $$(group.button).addClass("validated").click();
        ...
        if (enableCopyPrimaryWinnersGroup && enableCopyPrimaryWinnersGroup._current === "true") {
          var runoffMsg = $(".runoff-date-to-copy").val()
            ? " and RUNOFFS held on:\n" + $(".runoff-date-to-copy").val()
            : "";
          util.confirm("You are about to add ONLY THE WINNERS from ALL PRIMARIES held on:\n"+
            $(".primary-date-to-copy").val() + runoffMsg +
            " to\n" + electionControl.getElectionDesc() + ".\n\n" +
            "Ok?",
            function (button) {
              if (button === "Ok") {
    ...
    function onClickVotersEdge() {
      //var includeAnswers = $("#voters-edge-include-answers").prop("checked") ? "1" : "0";
      var csvType = $.radioVal("csv-type");
      $$("voters-edge-anchor").attr("href", "/admin/DownloadVotersEdgeCsv.aspx?election=" +
        getSelectedElection() /*+ "&answers=" + includeAnswers*/ + "&type=" + csvType)[0].click();
    }
    ...
    function getGeneralElectionDescriptionFailed() {
