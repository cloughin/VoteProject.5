...
function($, moment) {
  ...
  (function () {
    ...
    var isTouch = false;
    var curRootClass = $html.hasClass(touchClass) ? touchClass : "";

    function addtouchclass() {
      clearTimeout(isTouchTimer);
      isTouch = true;
      if (curRootClass != touchClass) {
        curRootClass = touchClass;
    ...
    function removetouchclass() {
      if (!isTouch && curRootClass == touchClass) {
        isTouch = false;
        curRootClass = "";
        $html.removeClass(touchClass);
      }
    }

    $html.on("touchstart", addtouchclass);
    $html.on("mouseover", removetouchclass);
  })();
  ...
  // if trigger needs time to settle
  (function () {
    $.fn.deferredTrigger = function (event, delay) {
      var $obj = this;
      setTimeout(function () { $obj.trigger(event) }, delay || 100);
      return this;
    };
  })();
...
//  };
  ...
  var isValidEmailRegex = /^(([^<>()\[\]\.,;:\s@\"]+(\.[^<>()\[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i;
