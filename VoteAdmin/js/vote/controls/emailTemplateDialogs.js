define([
    "jquery", "vote/util", "store", "jqueryui", "resizablecolumns",
    "stupidtable"
  ],
  function($, util, store) {
    ...
    var refreshDisplay = function() {
      ...
      var savedSort =
        util.saveSort($('table', $activeDialog), { col: ".name", dir: dirs.ASC });
      ...
      $.each(templates, function() {
        ...
        rows.push('<tr class="' +
          ...
          moddate.format("X") +
          '"><div class="moddate">' +
          moddate.format("M/D/YYYY h:mm:ss A") +
          '</div></td>' +
          '<td data-sort-value="' +
          lastuseddate.format("X") +
          '"><div class="lastuseddate">' +
          lastuseddisplay +
          '</div></td>' +
          '</tr>');
      });
      ...
      var tableHtml = '<table data-resizable-columns-id="' +
        resizeId +
        '"><thead><tr>' +
    ...
    var onClickMenuPublic = function(event) {
      ...
      if (item) {
        ...
        util.openAjaxDialog("Setting template to " +
    ...
    var initOpen = function() {
      ...
      $('.compatible-box input[type=checkbox]', $openDialog)
        .safeBind("change", onChangeCompatibleCheckbox);
    ...
    var openOpenDialog = function(options) {
      ...
      $(".compatible-box label", $openDialog)
        .toggleClass("disabled", $compatible.prop("disabled"));
      ...
      util.ajax({
        ...
        error: function(result) {
          util.closeAjaxDialog();
          util.alert(
            util.formatAjaxError(result, "Could not get template data from server"));
          closeOpenDialog();
        }
      });
    ...
    var doSave = function() {
      saveOptions.isPublic =
        $('input:radio[value="Public"]', $saveAsDialog).prop('checked');
      ...
      util.ajax({
        ...
        data: {
          ...
          emailType: saveOptions.emailType,
    ...
    var onSave = function() {
      ...
      if (saveOptions.name.length > maxTemplateNameLength) {
        util.alert("The Template Name cannot exceed " +
      ...
      if (exists) {
        util.confirm("A template with the name \"" +
          saveOptions.name +
