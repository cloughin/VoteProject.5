...
{
  public sealed class AdminElectionReport : ResponsiveReport
  {
    #region Private

    private ReportLevel _ReportLevel;
    private PlaceHolder _MultiCountySection;
    public int WinnersNotIdentified { get; private set; }
    ...
    private bool _IsMostRecentPastElectionOfType;
    ...
    private sealed class ElectionSort : ReportDataManager<DataRow>.OrderBy
    {
      ...
      {
        var result = row1.AlternateOfficeLevel().CompareTo(row2.AlternateOfficeLevel());
        if (result != 0) return result;
        result = row1.OfficeLevel().CompareTo(row2.OfficeLevel());
        if (result != 0) return result;
        result = string.Compare(row1.DistrictCode(), row2.DistrictCode(),
          StringComparison.OrdinalIgnoreCase);
        if (result != 0) return result;
        result = row1.OfficeOrderWithinLevel().CompareTo(row2.OfficeOrderWithinLevel());
        if (result != 0) return result;
        result = string.Compare(row1.OfficeLine1(), row2.OfficeLine1(),
          StringComparison.OrdinalIgnoreCase);
        if (result != 0) return result;
        return row1.OrderOnBallot().CompareTo(row2.OrderOnBallot());
      }
    }
    ...
    private sealed class OfficeOrderSort : ReportDataManager<DataRow>.OrderBy
    {
      public override int Compare(DataRow row1, DataRow row2)
      {
        var result = row1.OfficeOrderWithinLevel().CompareTo(row2.OfficeOrderWithinLevel());
        if (result != 0) return result;
        result = string.Compare(row1.OfficeLine1(), row2.OfficeLine1(),
          StringComparison.OrdinalIgnoreCase);
        if (result != 0) return result;
    ...
    private sealed class ElectionReportDataManager : RunningMateDataManager
    {
        ...
        FilterBy filterBy = null, OrderBy orderBy = null)
      {
        return GetDataSubset(filterBy, orderBy ?? new ElectionSort())
          .GroupBy(row => row.OfficeKey()).ToList();
      }
    }

    private abstract class OneCategory
    {
      protected AdminElectionReport ElectionReport;
      ...
      public void Generate(AdminElectionReport electionReport, string electionKey,
        bool isForAllStatesReport = false)
      {
        ...
        var offices = GetOffices().Where(g => g.First().ElectionsPoliticianKey() != null)
          .ToList();

        if (offices.Count <= 0) return;

        Control container;
        // only create a category wrapper if isForAllStatesReport
        if (isForAllStatesReport)
        {
          // ReSharper disable once PossibleNullReferenceException
          (new HtmlDiv().AddTo(ElectionReport.ReportContainer,
              "category-title accordion-header") as HtmlGenericControl).InnerHtml =
            GetCategoryTitle();
        ...
        foreach (var office in offices)
        {
          ...
          ReportOffice(officeContent, isRunningMateOffice, politicians,
            ElectionReport._DataManager, false, false, true);
        }
      }
      ...
      {
        return ElectionReport._DataManager.GetGroupedSubset(null, new OfficeOrderSort());
      }
    ...
    private sealed class USPresidentCategory : OneCategory
    {
      private class USPresidentFilter : ReportDataManager<DataRow>.FilterBy
      {
        ...
        public USPresidentFilter(string stateCode, string electionKey)
        {
          ...
          _IsPresidentialComparison = Elections.GetElectionTypeFromKey(electionKey) ==
            Elections.ElectionTypeUSPresidentialPrimary;
        }

        public override bool Filter(DataRow row)
        {
          return (_IsPresidentialComparison ||
              row.StateCode().IsEqIgnoreCase(_StateCode)) &&
      ...
      {
        return ElectionReport._DataManager.GetGroupedSubset(
    ...
    private sealed class USSenateCategory : OneCategory
    {
      private class USSenateFilter : ReportDataManager<DataRow>.FilterBy
      {
        ...
        public override bool Filter(DataRow row)
        {
          return row.StateCode().IsEqIgnoreCase(_StateCode) &&
      ...
      protected override IEnumerable<IGrouping<string, DataRow>> GetOffices()
      {
        return ElectionReport._DataManager.GetGroupedSubset(new USSenateFilter(StateCode));
      }

      protected override string GetOfficeTitle(DataRow officeInfo)
      {
        if (IsForAllStatesReport)
          return "US Senate (" + StateCache.GetStateName(StateCode) + ")";
        return "US Senate";
      }
    }
    ...
    private sealed class USHouseCategory : OneCategory
    {
      ...
      protected override IEnumerable<IGrouping<string, DataRow>> GetOffices()
      {
        return ElectionReport._DataManager.GetGroupedSubset(new USHouseFilter(StateCode),
          new USHouseSort());
      }
      ...
      private class USGovernorsFilter : ReportDataManager<DataRow>.FilterBy
      {
        ...
        public override bool Filter(DataRow row)
        {
          ...
          (row.AlternateOfficeClass() == OfficeClass.USGovernors ||
            ...
            row.AlternateOfficeClass() == OfficeClass.DCMayor) && !row.IsRunningMate();
        }
      }
    ...
    private sealed class StatewideCategory : OneCategory
    {
      private class StatewideFilter : ReportDataManager<DataRow>.FilterBy
      {
        ...
        public override bool Filter(DataRow row)
        {
          ...
          (Offices.StateOfficesWithoutLegislature.Contains(row.OfficeClass()) &&
            ...
            row.OfficeClass() == OfficeClass.StateParty) && !row.IsRunningMate();
        }
      }
      ...
      protected override IEnumerable<IGrouping<string, DataRow>> GetOffices()
      {
        return ElectionReport._DataManager.GetGroupedSubset(new StatewideFilter(StateCode));
      }
    ...
    private sealed class StateSenateCategory : OneCategory
    {
      ...
      protected override string GetOfficeTitle(DataRow officeInfo)
      {
        ...
        //    default:
        //      return "Board Member Ward " + officeInfo.DistrictCode().TrimStart('0');
        //  }
      ...
      ReportLevel reportLevel, Control multiCountySection = null)
    {
      ...
      if (localElectionDictionary.Count == 0) return 0;
      ...
      ReportLevel reportLevel)
    {
      ...
      if (countyElectionDictionary.Count == 0) return 0;
    ...
    private int CreateLowerLevelElectionReports(string electionKey)
    {
      switch (Elections.GetElectoralClass(electionKey))
      {
        case ElectoralClass.State:
          return GetCountyElections(ReportContainer, electionKey, _ReportLevel);

        case ElectoralClass.County:
          return GetLocalElections(ReportContainer, electionKey, _ReportLevel,
            _MultiCountySection);
    ...
    private void CreateMasterOfficeLinks(Control officeContent, DataRow officeInfo, int winners)
    {
      ...
      var positions = GetElectionPositions(officeInfo);
      if (_IsMostRecentPastElectionOfType)
      {
        if (winners == positions)
          new HtmlSpan { InnerText = "Winners identified" }
            .AddTo(div);
        else
        {
          if (winners < positions)
            CreateAdminOfficeWinnerAnchor(officeInfo.ElectionKey(), officeInfo.OfficeKey(),
              $"Identify Winners ({winners} identified)").AddTo(div, "boldred");
          else
            new HtmlSpan { InnerText = "Too many winners identified" }
              .AddTo(div, "boldred");
          WinnersNotIdentified++;
        }
        new HtmlSpan { InnerText = $"Positions: {positions}" }
          .AddTo(div);
      }
    }

    private Control GenerateReport(string electionKey,
      ReportLevel reportLevel = ReportLevel.DefaultLevel,
      PlaceHolder multiCountySection = null)
    {
      if (reportLevel == ReportLevel.DefaultLevel)
      {
        if (Elections.IsLocalElection(electionKey)) _ReportLevel = ReportLevel.LocalLevel;
        else if (Elections.IsCountyElection(electionKey))
          _ReportLevel = ReportLevel.CountyLevel;
        else _ReportLevel = ReportLevel.StateLevel;
      }
      else _ReportLevel = reportLevel;
      _MultiCountySection = multiCountySection;

      ReportUser = ReportUser.Master;
      _IsMostRecentPastElectionOfType = false;
      if (Elections.GetElectionDateFromKey(electionKey) < DateTime.UtcNow)
      {
        var recentElections =
          Elections.GetLatestElectionsByStateCodeElectionType(Elections.GetStateCodeFromKey(electionKey),
            Elections.GetElectionTypeFromKey(electionKey));
        _IsMostRecentPastElectionOfType = recentElections == null ||
          recentElections.Contains(Elections.GetStateElectionKeyFromKey(electionKey), StringComparer.OrdinalIgnoreCase);
      }
      ...
      switch (electoralClass)
      {
        case ElectoralClass.USPresident:
          if (Elections.GetElectionTypeFromKey(electionKey) ==
            Elections.ElectionTypeUSPresidentialPrimary /*|| 
            Elections.GetStateCodeFromKey(electionKey) == "US"*/)
      ...
      switch (electoralClass)
      {
        case ElectoralClass.State:
        case ElectoralClass.County:
          WinnersNotIdentified += CreateLowerLevelElectionReports(electionKey);
          break;
      }

      if (electoralClass == ElectoralClass.State && WinnersNotIdentified > 0)
        ReportContainer.Controls.AddAt(0,
          new HtmlP
          {
            InnerText =
              $"Winners have not been identified for {WinnersNotIdentified} contests"
          }.AddCssClasses("boldred"));
    ...
    private void ReportReferendums(string electionKey)
    {
      ...
      // ReSharper disable once PossibleNullReferenceException
      (new HtmlDiv().AddTo(ReportContainer, "category-title accordion-header") as
        HtmlGenericControl).InnerHtml = "Referendums and Ballot Measures";
    ...
    {
      var reportObject = new AdminElectionReport();
