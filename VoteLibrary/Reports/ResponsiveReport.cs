...
{
  public class ResponsiveReport : Report
  {
    #region Private

    private static void CreateCandidateCell(Control container, DataRow politician,
      bool includeKey = false, bool includeCheck = false, bool forAdmin = false)
    {
      if (politician != null)
        ReportPolitician(container, politician, politician.IsWinner(),
          politician.IsIncumbent(), null, includeKey, includeCheck, forAdmin);
    ...
    protected abstract class RunningMateDataManager : ReportDataManager<DataRow>
    {
      ...
      {
        return DataList.FirstOrDefault(row => row.OfficeKey().IsEqIgnoreCase(officeKey) &&
          row.RunningMateKey().IsEqIgnoreCase(politicianKey) && row.IsRunningMate());
      }
    }

    protected static void CreateCompareOrIntroAnchor(Control container,
      IList<DataRow> politicians, string electionKey, string officeKey)
    {
      if (politicians.Count > 1)
        CreateCompareTheCandidatesAnchor(electionKey, officeKey).AddTo(container);
      else
      {
      ...
      string referendumKey, string anchorText)
    {
      ...
      {
        HRef = UrlManager.GetReferendumPageUri(electionKey, referendumKey).ToString(),
    ...
    protected static void FormatAge(Control infoContainer, DataRow politician)
    {
      ...
      if (!IsNullOrEmpty(age))
        new HtmlDiv {InnerText = age}.AddTo(infoContainer, "candidate-age");
    }
    ...
    {
      if (isIncumbent) container.AddCssClasses("incumbent");
      ...
      else span.InnerText = politicianName;
    ...
    protected static void FormatMoreInfoLink(Control infoContainer, DataRow politician, bool forAdmin = false)
    {
      var moreInfoDiv = new HtmlDiv().AddTo(infoContainer, "candidate-more-info no-print");
      CreatePoliticianIntroAnchor(politician, 
        forAdmin ? "View Public Intro Page" : "More information", Empty, "politician")
        .AddTo(moreInfoDiv);
    }
    ...
    protected static void FormatPostalAddress(Control infoContainer, DataRow politician)
    {
      ...
      if (!IsNullOrEmpty(streetAddress) && !IsNullOrEmpty(cityStateZip))
      {
      ...
      var addressContainer = new HtmlDiv().AddTo(infoContainer, "candidate-address");
    ...
    protected static void FormatWebAddress(Control container, DataRow politician)
    {
      ...
      {
        var websiteContainer = new HtmlDiv().AddTo(container, "candidate-website");
        var wrapper = new HtmlSpan().AddTo(websiteContainer, "no-print");
        website.AddTo(wrapper);
        new HtmlSpan {InnerText = webAddress}.AddTo(websiteContainer, "only-print print-url");
      }
    }
    ...
    protected static int GetElectionPositions(DataRow officeInfo)
    {
      return GetElectionPositions(officeInfo.ElectionKey(), officeInfo);
    }
    ...
    protected static void ReportOffice(Control container, bool isRunningMateOffice,
      ...
      bool includePoliticianKey = false, bool includeCheck = false,
      bool forAdmin = false)
    {
      var officeInfo = politicians[0];
      container.AddCssClasses("candidates-" + politicians.Count);

      foreach (var row in politicians.Where(p => p.ElectionsPoliticianKey() != null))
      {
        var ticketContainer = isRunningMateOffice
          ? new HtmlDiv().AddTo(container, "office-ticket")
          : container;

        CreateCandidateCell(ticketContainer, row, includePoliticianKey, includeCheck,
          forAdmin);

        if (!isRunningMateOffice) continue;

        var runningMate =
          dataManager.GetRunningMate(officeInfo.OfficeKey(), row.RunningMateKey());

        CreateCandidateCell(ticketContainer, runningMate, false, false, forAdmin);
      }
    }

    protected void ReportOneReferendum(Control container, DataRow row,
      bool forBallotPage = false)
    {
      var formattedTitle = row.ReferendumTitle().ReplaceNewLinesWithSpans();
      new HtmlDiv {InnerHtml = formattedTitle}.AddTo(container,
        "referendum-title accordion-header");

      var contentDiv =
        new HtmlDiv().AddTo(container, "referendum-content accordion-content");
      ...
      if (!forBallotPage && row.IsResultRecorded())
        new HtmlDiv().AddTo(contentDiv,
          row.IsPassed() ? "referendum-passed" : "referendum-failed");

      var formattedDescription = row.ReferendumDescription().ReplaceNewLinesWithSpans();
      var referendumDetailDesc = row.ReferendumDescription() != Empty
        ? formattedDescription
        : formattedTitle;

      new HtmlDiv {InnerHtml = referendumDetailDesc}.AddTo(contentDiv,
      ...
      if (ReportUser == ReportUser.Master)
        CreateAdminReferendumAnchor(row.ElectionKey(), row.ReferendumKey(),
          "Edit description, details and full text").AddTo(anchorDiv);
      else
        CreateReferendumPageAnchor(row.ElectionKey(), row.ReferendumKey(),
          "Description, details and full text").AddTo(anchorDiv);
    }

    protected static void ReportPolitician(Control container, DataRow politician,
      bool isWinner, bool isIncumbent, string heading = null, bool includeKey = false,
      bool includeCheck = false, bool forAdmin = false)
    {
      ...
      FormatCandidateNameAndParty(nameContainer, politician, isIncumbent, true, false,
        forAdmin, includeCheck ? "clicker" : null);

      var imageContainer = new HtmlDiv().AddTo(content, "candidate-image");
      if (forAdmin)
        CreatePoliticianImageAnchor(
            //UrlManager.GetAdminUri(SecurePoliticianPage.GetUpdateIssuesPageUrl(politicianKey))
            //  .ToString(),
            SecurePoliticianPage.GetUpdateIssuesPageUrl(politicianKey),
            politicianKey, ImageSize100, "Edit Issue Topic Responses", "politician", true)
          .AddTo(imageContainer);
      else
        CreatePoliticianImageTag(politicianKey, ImageSize100, false, Empty)
          .AddTo(imageContainer);

      var infoContainer = new HtmlDiv().AddTo(content, "candidate-info");

      if (isWinner) new HtmlDiv().AddTo(infoContainer, "candidate-is-winner");
      ...
      FormatMoreInfoLink(infoContainer, politician, forAdmin);
    }
