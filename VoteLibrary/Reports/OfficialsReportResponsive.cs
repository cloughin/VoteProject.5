...
{
  public class OfficialsReportResponsive : ResponsiveReport
  {
    #region Private
    ...
    private static void ReportOfficeHeadingAndIncumbent(Control container, string officeTitle,
      DataRow politician)
    {
    ...
    protected sealed class CountyCategory : OneCategory
    {
      private class CountyFilter : ReportDataManager<DataRow>.FilterBy
      {
        public override bool Filter(DataRow row)
        {
          return !IsNullOrEmpty(row.CountyCode()) && !row.IsRunningMate();
        }
      }
      ...
      protected override IList<IGrouping<string, DataRow>> GetOffices()
      {
        return OfficialsReport.DataManager.GetGroupedSubset(new CountyFilter(/*StateCode*/));
      }
    ...
    protected class LocalCategory : OneCategory
    {
      protected class LocalFilter : ReportDataManager<DataRow>.FilterBy
      {
        public override bool Filter(DataRow row)
        {
          return !IsNullOrEmpty(row.LocalKey()) && !row.IsRunningMate();
        }
      }
      ...
      protected override IList<IGrouping<string, DataRow>> GetOffices()
      {
        return OfficialsReport.DataManager.GetGroupedSubset(new LocalFilter(/*StateCode*/));
      }
