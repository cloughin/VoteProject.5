...
namespace DB
{
  namespace VoteCache
  {
    ...
    public static partial class VoteCacheDb
    {
      ...
      static VoteCacheDb()
      {
        ...
        if (IsNullOrWhiteSpace(connectionString))
          connectionString = WebConfigurationManager.AppSettings["CnVoteCache.MySql"];
        if (!IsNullOrWhiteSpace(connectionString))
    ...
    #region votecache.DonationNags2

    public partial class DonationNags2Row : DataRow
    {
      internal DonationNags2Row(DataRowBuilder rb) : base(rb) {}
    ...
    [Serializable]
    public partial class DonationNags2Table : TypedTableBase<DonationNags2Row>
    {
      public DonationNags2Row this[int index] { get { return (DonationNags2Row)(this.Rows[index]); } }

      public int Count { get { return this.Rows.Count; } }

      public void AddRow(DonationNags2Row row) { this.Rows.Add(row); }

      protected override DataTable CreateInstance() { return new DonationNags2Table(); }

      new public DonationNags2Row NewRow() { return (DonationNags2Row) (base.NewRow()); }

      protected override DataRow NewRowFromBuilder(DataRowBuilder rb)
      { return new DonationNags2Row(rb); }

      protected override Type GetRowType()
      { return typeof(DonationNags2Row); }

      public void RemoveRow(DonationNags2Row row) { this.Rows.Remove(row); }

      public DonationNags2Row NewRow(Int32 id, String message)
      {
        DonationNags2Row row = NewRow();
        row.Id = id;
        row.Message = message;
        return row;
      }

      public void AddRow(Int32 id, String message)
      {
        DonationNags2Row row = NewRow(id, message);
        AddRow(row);
      }

      public DonationNags2Row NewRow(String message)
      {
        DonationNags2Row row = NewRow();
        row.Message = message;
        return row;
      ...
      {
        DonationNags2Row row = NewRow(message);
        AddRow(row);
      }
      ...
      public DonationNags2Table() : this(ColumnSet.All) { }

      public DonationNags2Table(ColumnSet columnSet)
      {
        this.TableName = "DonationNags2";
        switch (columnSet)
        {
          case ColumnSet.All:
            InitAllColumns();
      ...
      protected DonationNags2Table(SerializationInfo info, StreamingContext context) : base(info, context) { }
    }

    public static partial class DonationNags2
    {
      public const string TableName = "DonationNags2";

      public enum Column { Id, Message }
      ...
      {
        object result;
        string cmdText = "SELECT COUNT(*) FROM DonationNags2";
        DbCommand cmd = VoteCacheDb.GetCommand(cmdText, commandTimeout);
        result = VoteCacheDb.ExecuteScalar(cmd);
        return Convert.ToInt32(result);
      }
      ...
      public static string SelectAllCommandText
      {
        get
        {
          return "SELECT Id,Message FROM DonationNags2";
        }
      }

      public static DonationNags2Table GetAllData(int commandTimeout = -1)
      {
        string cmdText = "SELECT Id,Message FROM DonationNags2";
        DbCommand cmd = VoteCacheDb.GetCommand(cmdText, commandTimeout);
        return FillTable(cmd, DonationNags2Table.ColumnSet.All);
      }

      public static DonationNags2Table FillTable(DbCommand command)
      {
        return FillTable(command, DonationNags2Table.ColumnSet.All);
      }

      [SuppressMessage("Microsoft.Reliability", "CA2000")]
      public static DonationNags2Table FillTable(DbCommand command, DonationNags2Table.ColumnSet columnSet)
      {
        using (DbConnection cn = VoteCacheDb.GetOpenConnection())
        {
          command.Connection = cn;
          DonationNags2Table table = new DonationNags2Table(columnSet);
          DbDataAdapter adapter = VoteCacheDb.GetDataAdapter(command);
          adapter.Fill(table);
          return table;
        }
      ...
      public static string GetSelectCommandText(DonationNags2Table.ColumnSet columnSet)
      {
        switch(columnSet)
        {
          case DonationNags2Table.ColumnSet.All:
            return SelectAllCommandText;
      ...
      static DonationNags2()
      {
        ColumnNameDictionary = ColumnDictionary.ToDictionary(kvp => kvp.Value, kvp => kvp.Key);
      }
    }

    #endregion votecache.DonationNags2
