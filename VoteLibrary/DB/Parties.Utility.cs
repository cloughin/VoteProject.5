...
namespace DB.Vote
{
  public partial class Parties
  {
    ...
    {
      if (partyKey == null || partyKey.Length < 2) return Empty;
    ...
    public static string GetNationalPartyDescription(string partyCode,
      string def = null)
    {
      if (def == null) def = "Unknown National Party";
      ...
      string def = "Unknown Party")
    {
      ...
      if (IsNullOrWhiteSpace(partyCode)) return def;
      var info = GetNationalPartyInfo(partyCode);
      if (info != null)
        if (!info.IsSpecial)
          return info.Description;
      return IsNullOrWhiteSpace(partyName)
        ? def
        : partyName;
    }
      ...
      string firstItem = "--- Select a party ---")
    {
      ...
      if (!IsNullOrWhiteSpace(firstItem)) dropDownList.AddItem(firstItem, " ");
