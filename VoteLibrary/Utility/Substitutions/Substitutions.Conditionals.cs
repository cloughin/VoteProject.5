  ...
  internal sealed class Conditionals
  {
    ...
    private sealed class Conditional
    {
      ...
      internal object ParseComparand(string predicate)
      {
        ...
        if (int.TryParse(predicate, out var intValue)) return intValue;
        if (DateTime.TryParse(predicate, out var dateValue)) return dateValue.Date;
          ...
          default:
          {
            ...
            switch (@operator)
            {
              ...
              case Operator.NotEmpty:
                if (!IsNullOrWhiteSpace(predicate))
    ...
    private bool EvaluateCondition(IfBlock ifBlock)
    {
      ...
      switch (ifBlock.If.Operator)
      {
        ...
        case Operator.Ne:
          ...
          int comparison;
          if (testValue is int i) comparison = i.CompareTo(ifBlock.If.Comparand);
        ...
        case Operator.Match:
          truth = Compare(ifBlock.If.Comparand as string, testString,
    ...
    private static void EvaluateFail(Conditional conditional)
    {
      ...
      else
      {
        ...
        if (!IsNullOrWhiteSpace(msg)) exceptionMsg += ": " + msg;
      }
