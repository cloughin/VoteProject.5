...
namespace Vote
{
  public static class StateCache
  {
      ...
      foreach (var row in table)
      {
        ...
        foreach (
          var trimmed in abbreviations.Select(abbreviation => abbreviation.Trim())
            .Where(trimmed => !IsNullOrWhiteSpace(trimmed)))
    ...
    {
      if (stateCode != null && StateCodeDictionary.TryGetValue(stateCode, out var info))
    ...
    {
      return StateAbbreviationDictionary.TryGetValue(stateName, out var info)
        ? info.StateCode
        : null;
    }
    ...
    {
      if (code != null && StateCodeDictionary.TryGetValue(code, out var info))
    ...
    {
      return LdsStateCodeDictionary.TryGetValue(ldsStateCode, out var info)
        ? info.StateCode
        : null;
    }
