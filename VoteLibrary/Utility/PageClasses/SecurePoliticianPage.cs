...
namespace Vote
{
  public class SecurePoliticianPage : SecurePage
  {
    ...
    public static string GetPoliticianPublicPageUrl(string pageName, string politicianKey = null)
    {
      if (IsNullOrWhiteSpace(politicianKey))
        politicianKey = UserSecurityClass == PoliticianSecurityClass
          ? UserPoliticianKey
          : PoliticianKeyFromSecurePoliticianPage;
      var url = Uri.UriSchemeHttps + "://" +
        //UrlManager.GetStateHostNameAndPort(Politicians.GetStateCodeFromKey(politicianKey)) +
        UrlManager.SiteHostNameAndPort +
        "/" + pageName + ".aspx?id=" + politicianKey;
    ...
    public void UpdatePoliticianAnswerNew(string questionKey, int sequence,
      string newValue, string source, DateTime dateStamp, string youTubeUrl,
      string youTubeDescription, TimeSpan youTubeRunningTime, string youTubeSource,
      string youTubeSourceUrl, DateTime youTubeDate, string facebookVideoUrl,
      string facebookVideoDescription, TimeSpan facebookVideoRunningTime,
      DateTime facebookVideoDate)
    {
      UpdatePoliticianAnswerNew(PoliticianKey, questionKey, sequence, newValue, source,
        dateStamp, youTubeUrl, youTubeDescription, youTubeRunningTime, youTubeSource,
        youTubeSourceUrl, youTubeDate, facebookVideoUrl, facebookVideoDescription,
        facebookVideoRunningTime, facebookVideoDate);
    }
    ...
    public void LogPoliticianAnswerChange(string questionKey, int sequence, string oldAnswer,
      string newAnswer, string source)
    {
      LogDataChange.LogUpdate(Answers2.Column.Answer, oldAnswer, newAnswer, source,
    ...
    public static string UpdateInfoPageUrl => GetPoliticianFolderPageUrl("updateinfo");
    ...
    public static string GetPoliticianName()
    {
      ...
      return page == null ? Empty : page.PoliticianName;
    }

    public static string GetPoliticianKey()
    {
      var page = GetPage<SecurePoliticianPage>();
      return page == null ? Empty : page.PoliticianKey;
    }
    ...
    {
      base.OnPreLoad(e);
      PoliticianKey = FindPoliticianKey();
    ...
    protected override void OnPreRender(EventArgs e)
    {
      ...
      var text = IsNullOrWhiteSpace(PoliticianKey)
